...
app.get('/v1/automations/webhookWanAnimate', async (req, res) => {
  ...
  try {
    ...
    const videoUrl = vidAttach[0]?.url || fields['sourceVideoUrl'] || fields['video'] || req.query.video;
    ...
    await patchAirtableRecord(baseId, requestedTbl, recordId, {
      [errField]: String(err.message),
      [statusField]: 'Error',
    });
    res.status(500).json({ ok: false, error: err.message });
  }
...
app.get('/v1/automations/webhookWan25i2v', async (req, res) => {
  ...
  try {
    ...
    const taskIds = await Promise.all(
      Array.from({ length: desired }, () => submitWan25_i2v({ image: img, prompt, duration, resolution, seed }))
    );
